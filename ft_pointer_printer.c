...
/*   Updated: 2024/06/16 23:42:44 by tamatsuu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
size_t	ft_print_pointer(t_format *fmt, uintptr_t i, int fd)
{
	...
	if (fmt->flg & MN_FLG)
	{
		...
		ret += ft_putptr_prec_fd(fmt, i, ft_putnbr_base_cnt(i, HXDCML_S), fd);
...
{
	size_t	ret;
	ret = ft_putnbr_base(17, HXDCML_S, 1);
	printf ("%ld\n",ret);
}
